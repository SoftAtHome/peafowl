...
TEST(SSLTest, Generic) {
  ...
  getProtocols("./pcaps/ssl-4.pcap", protocols,
               state); // Short flows and we do not see syn. We detect only if we disable TCP reordering.
...
TEST(SSLTest, Tags) {
  pfwl_state_t *state = pfwl_init();
  pfwl_field_string_tags_add_L7(state, PFWL_FIELDS_L7_SSL_CERTIFICATE, "snakeoil.dom", PFWL_FIELD_MATCHING_SUFFIX,
                                "TAG_SUFFIX");
  ...
  getProtocols("./pcaps/ssl-2.cap", protocols, state, [&](pfwl_status_t, pfwl_dissection_info_t r) {
    for (size_t i = 0; i < r.l7.tags_num; i++) {
      if (r.l7.tags_num && !strcmp(r.l7.tags[i], "TAG_SUFFIX")) {
        foundSni = true;
      }
    }
  });
...
TEST(SSLTest, JA3) {
  ...
  bool versionFound = false, ciphersFound = false, extensionsFound = false, curvesFound = false,
       curvesPointsFound = false, jaFound = false;

  bool versionFoundSer = false, ciphersFoundSer = false, extensionsFoundSer = false, curvesFoundSer = false,
       curvesPointsFoundSer = false, jaFoundSer = false;
  getProtocols("./pcaps/ssl-3.pcap", protocols, state, [&](pfwl_status_t status, pfwl_dissection_info_t r) {
    pfwl_string_t field;
    int64_t fieldNum;
    int64_t htype;
      ...
      if (!pfwl_field_number_get(r.l7.protocol_fields, PFWL_FIELDS_L7_SSL_HANDSHAKE_TYPE, &htype)) {
        if (htype == 0x01) {
          ...
          if (!pfwl_field_string_get(r.l7.protocol_fields, PFWL_FIELDS_L7_SSL_CIPHER_SUITES, &field)) {
            EXPECT_EQ(strncmp((const char *) field.value,
                              "49172-49162-57-56-55-54-136-135-134-133-49167-49157-53-132-49171-49161-51-50-49-48-154-"
                              "153-152-151-69-68-67-66-49166-49156-47-150-65-7-49169-49159-49164-49154-5-4-49170-49160-"
                              "22-19-16-13-49165-49155-10-255",
                              field.length),
                      0);
            ciphersFound = true;
          }
...
TEST(SSLTest, JA3Tags) {
  ...
  getProtocols("./pcaps/whatsapp_login_call.pcap", protocols, state, [&](pfwl_status_t, pfwl_dissection_info_t r) {
    for (size_t i = 0; i < r.l7.tags_num; i++) {
      if (r.l7.tags_num && !strcmp(r.l7.tags[i], "Used by many programs on OSX,apple.WebKit.Networking")) {
        found = true;
      }
    }
  });
